...
static uint32_t gps_rtc_last_check_ms;
static uint32_t gps_rtc_check_count;
...
static uint32_t latest_gps_satellites_visible_ms;
static uint32_t latest_gps_satellites_used_ms;
static uint8_t latest_gps_fix_valid;
static uint8_t latest_gps_speed_valid;
static uint8_t latest_gps_altitude_valid;
static uint8_t latest_gps_satellites_visible;
static uint8_t latest_gps_satellites_used;
static uint8_t latest_gps_satellites_visible_valid;
static uint8_t latest_gps_satellites_used_valid;
...
/*
 * The enabled external PCF85263A is the primary calendar. The source label and
 * GPS comparison status are included in SWV, ESP32, and RS232 output so the
 * operator can confirm that the retained calendar remains synchronized.
 */
static uint8_t stm32_rtc_ready;
static uint8_t pcf85263a_rtc_ready;
#if ENABLE_SWV_DEBUG_OUTPUT
static uint32_t rtc_last_print_ms;
#endif
static const char *rtc_time_source = "UNSYNCED";
...
static uint8_t GPS_ParseCountField(const char *sentence, uint8_t field_index,
                                   uint8_t *count);
...
static uint8_t STM32_RTC_Set(const TelemetryDateTime_t *date_time)
    __attribute__((unused));
#endif
static uint8_t TelemetryRTC_Read(TelemetryDateTime_t *date_time);
static uint8_t TelemetryRTC_Set(const TelemetryDateTime_t *date_time);
static uint8_t TelemetryDateTime_IsValid(const TelemetryDateTime_t *date_time);
static int32_t TelemetryDateTime_DifferenceSeconds(
    const TelemetryDateTime_t *left, const TelemetryDateTime_t *right);
...
#if ENABLE_SWV_DEBUG_OUTPUT
static void TelemetryRTC_Task(void);
#endif
    ...
    const TelemetryDateTime_t *left, const TelemetryDateTime_t *right)
{
  ...
  if ((TelemetryDateTime_IsValid(left) == 0U) ||
      (TelemetryDateTime_IsValid(right) == 0U)) return 2147483647L;

  left_days = (int64_t)(left->year - 2000U) * 365LL +
              (int64_t)((left->year - 2000U + 3U) / 4U) +
              days_before_month[left->month - 1U] + left->day - 1U;
  right_days = (int64_t)(right->year - 2000U) * 365LL +
               (int64_t)((right->year - 2000U + 3U) / 4U) +
               days_before_month[right->month - 1U] + right->day - 1U;
  if (((left->year % 4U) == 0U) && (left->month > 2U)) left_days++;
  if (((right->year % 4U) == 0U) && (right->month > 2U)) right_days++;

  difference = (left_days - right_days) * 86400LL +
      ((int64_t)left->hour - right->hour) * 3600LL +
      ((int64_t)left->minute - right->minute) * 60LL +
      (int64_t)left->second - right->second;
  if (difference > 2147483647LL) return 2147483647L;
  if (difference < -2147483647LL) return -2147483647L;
...
static void TelemetryRTC_Task(void)
{
  ...
  if ((now - rtc_last_print_ms) < 1000U) return;
  rtc_last_print_ms = now;

  TelemetryDateTime_t date_time = {0};
  char text[24];
  char uptime[24];
  BoardUptime_Format(now, uptime, sizeof(uptime));
  if (TelemetryRTC_Read(&date_time) != 0U)
  {
    TelemetryDateTime_Format(&date_time, text, sizeof(text));
    printf("[RTC] %s device=%s time_source=%s gps_sync_valid=%u drift_s=%ld gps_checks=%lu gps_sets=%lu uptime=%s\r\n",
           text,
           ...
           "STM32_INTERNAL",
#endif
           rtc_time_source, gps_rtc_sync_valid,
           (long)gps_rtc_last_drift_seconds, gps_rtc_check_count,
           gps_rtc_sync_count, uptime);
  }
  else
  {
    printf("[RTC] read failed device=%s\r\n",
#if ENABLE_EXTERNAL_PCF85263A_RTC
           "PCF85263A");
#else
           "STM32_INTERNAL");
...
static void VehicleSpeed_UpdateFromSources(void)
{
  ...
  if (latest_gps_satellites_visible_valid &&
      ((now - latest_gps_satellites_visible_ms) > GPS_SATELLITE_VALID_MS))
    latest_gps_satellites_visible_valid = 0U;
  if (latest_gps_satellites_used_valid &&
      ((now - latest_gps_satellites_used_ms) > GPS_SATELLITE_VALID_MS))
    latest_gps_satellites_used_valid = 0U;
 ...
 * the 7-bit 0x51 target address shifted left by one. Hardware requires I2C1,
 * a backup supply, and the PCF85263A's 32.768 kHz crystal.
 */
...
static uint8_t Future_PCF85263A_BcdToBin(uint8_t value)
{
  uint8_t high = (uint8_t)((value >> 4) & 0x0FU);
  uint8_t low = (uint8_t)(value & 0x0FU);
  return ((high > 9U) || (low > 9U)) ? 0xFFU :
      (uint8_t)(high * 10U + low);
}

static uint8_t Future_PCF85263A_ReadDateTime(TelemetryDateTime_t *date_time)
{
  uint8_t registers[8];

  if (date_time == NULL) return 0U;
  memset(date_time, 0, sizeof(*date_time));
  if (Future_PCF85263A_ReadRaw(registers) == 0U) return 0U;
  /* Seconds bit 7 is OS; a set flag means oscillator integrity was lost. */
  if ((registers[1] & 0x80U) != 0U) return 0U;

  date_time->second = Future_PCF85263A_BcdToBin(registers[1] & 0x7FU);
  date_time->minute = Future_PCF85263A_BcdToBin(registers[2] & 0x7FU);
  date_time->hour = Future_PCF85263A_BcdToBin(registers[3] & 0x3FU);
  date_time->day = Future_PCF85263A_BcdToBin(registers[4] & 0x3FU);
  date_time->weekday = (registers[5] == 0U) ? RTC_WEEKDAY_SUNDAY :
      (uint8_t)(registers[5] & 0x07U);
  date_time->month = Future_PCF85263A_BcdToBin(registers[6] & 0x1FU);
  date_time->year = 2000U + Future_PCF85263A_BcdToBin(registers[7]);
  if (TelemetryDateTime_IsValid(date_time) == 0U) return 0U;
  date_time->valid = 1U;
  pcf85263a_rtc_ready = 1U;
  return 1U;
}
...
{
#if ENABLE_EXTERNAL_PCF85263A_RTC
  if (Future_PCF85263A_ReadDateTime(date_time) != 0U) return 1U;
#endif
#if ENABLE_STM32_INTERNAL_RTC
  if (STM32_RTC_Read(date_time) != 0U) return 1U;
#endif
  if (date_time != NULL) memset(date_time, 0, sizeof(*date_time));
  return 0U;
}
...
#if ENABLE_EXTERNAL_PCF85263A_RTC
  configured = 1U;
  if (Future_PCF85263A_Set(date_time) == 0U) success = 0U;
#endif
#if ENABLE_STM32_INTERNAL_RTC
  configured = 1U;
  if (STM32_RTC_Set(date_time) == 0U) success = 0U;
#endif
  return ((configured != 0U) && (success != 0U)) ? 1U : 0U;
}
...
#endif
#if ENABLE_EXTERNAL_PCF85263A_RTC
  pcf85263a_rtc_ready = Future_PCF85263A_Init();
  if (pcf85263a_rtc_ready != 0U)
  {
    TelemetryDateTime_t retained = {0};
    if (Future_PCF85263A_ReadDateTime(&retained) != 0U)
      rtc_time_source = "PRESERVED";
  }
  printf("[RTC BOOT] PCF85263A init=%u calendar_valid=%u; waiting for GPS UTC\r\n",
         pcf85263a_rtc_ready,
         (strcmp(rtc_time_source, "PRESERVED") == 0) ? 1U : 0U);
#endif
}
...
#endif
#if ENABLE_EXTERNAL_PCF85263A_RTC
  /* The decoded RTC and GPS comparison are printed by TelemetryRTC_Task(). */
#endif
}
#endif
...
static uint8_t GPS_IsNavSentence(const char *sentence)
{
   ...
   * contain position, ground speed, or satellite-search values used by
   * telemetry.
   */
  ...
  return ((strncmp(&sentence[3], "RMC", 3U) == 0) ||
          ...
          (strncmp(&sentence[3], "VTG", 3U) == 0) ||
          (strncmp(&sentence[3], "GSV", 3U) == 0)) ? 1U : 0U;
}
                                   ...
                                   uint8_t *count)
{
  ...
  if ((count == NULL) ||
      (GPS_GetField(sentence, field_index, field, sizeof(field)) == 0U) ||
      (field[0] == '\0')) return 0U;
  parsed = strtoul(field, &end, 10);
  if ((end == field) || (*end != '\0') || (parsed > 99UL)) return 0U;
...
static void GPS_UpdateNavFromSentence(const char *sentence)
{
   ...
   * GGA: latitude/longitude/elevation and satellites used by the fix.
   * GSV: GPS satellites visible while the receiver searches for a fix.
  ...
  if ((strncmp(&sentence[3], "GGA", 3U) == 0) &&
      GPS_ParseCountField(sentence, 7U, &latest_gps_satellites_used))
  {
    latest_gps_satellites_used_valid = 1U;
    latest_gps_satellites_used_ms = now;
  }
  if ((strncmp(&sentence[3], "GSV", 3U) == 0) &&
      (sentence[1] == 'G') &&
      ((sentence[2] == 'P') || (sentence[2] == 'N')) &&
      GPS_ParseCountField(sentence, 3U, &latest_gps_satellites_visible))
  {
    latest_gps_satellites_visible_valid = 1U;
    latest_gps_satellites_visible_ms = now;
  }
       ...
       ((now - gps_rtc_last_check_ms) >= GPS_RTC_SYNC_PERIOD_MS)))
  {
    ...
    uint8_t rtc_read_valid = TelemetryRTC_Read(&rtc_date_time);
    uint8_t correction_required;
    uint32_t check_number = gps_rtc_check_count + 1U;

    gps_rtc_last_drift_seconds = (rtc_read_valid != 0U) ?
        TelemetryDateTime_DifferenceSeconds(&rtc_date_time, &gps_date_time) :
        2147483647L;
    correction_required = ((gps_rtc_check_count == 0U) ||
        (rtc_read_valid == 0U) ||
        (gps_rtc_last_drift_seconds > GPS_RTC_MAX_DRIFT_SECONDS) ||
        (gps_rtc_last_drift_seconds < -GPS_RTC_MAX_DRIFT_SECONDS)) ? 1U : 0U;
    TelemetryDateTime_Format(&gps_date_time, gps_time_text,
                             sizeof(gps_time_text));

    if (correction_required != 0U)
    {
      if (TelemetryRTC_Set(&gps_date_time) != 0U)
      {
        ...
        printf("[GPS RTC] set PCF85263A to %s UTC check=%lu set=%lu\r\n",
               gps_time_text, check_number, gps_rtc_sync_count);
      }
      else
      {
        gps_rtc_sync_valid = 0U;
        printf("[GPS RTC] PCF85263A synchronization failed check=%lu\r\n",
               check_number);
      }
    }
    else
  ...
  if ((now - gps_last_nav_print_ms) >= GPS_NAV_PRINT_PERIOD_MS)
  {
    ...
    printf("[GPS] nav=%lu filtered=%lu checksum_err=%lu fix=%u sats_visible=%u visible_valid=%u sats_used=%u used_valid=%u lat=%s lon=%s alt=%s m alt_valid=%u speed=%lu.%02lu mph\r\n",
           gps_valid_count, gps_filtered_count, gps_checksum_error_count,
           latest_gps_fix_valid, latest_gps_satellites_visible,
           latest_gps_satellites_visible_valid, latest_gps_satellites_used,
           latest_gps_satellites_used_valid, lat_text, lon_text, altitude_text,
...
static void GPS_ProcessByte(uint8_t byte)
{
  ...
  if (byte == '\n' && gps_nmea_index != 0U)
  {
    ...
    {
      /* Only checksum-valid RMC/GGA/VTG/GSV sentences reach telemetry. */
      gps_valid_count++;
...
static void ESP32_ExecuteCommand(const char *id, const char *verb)
{
  ...
  if (strncmp(verb, "SET_RTC,", 8U) == 0)
  {
    ...
    requested.weekday = TelemetryDateTime_CalcWeekday(requested.year,
                                                       requested.month,
                                                       requested.day);
    if (TelemetryRTC_Set(&requested) == 0U)
    {
...
static void ESP32_Task(void)
{
  ...
  {
    uint8_t rtc_valid = TelemetryRTC_Read(&date_time);
    VehicleSpeed_UpdateFromSources();
    uint32_t vehicle_hundredths =
        (uint32_t)(latest_vehicle_speed_mph * 100.0f + 0.5f);
    uint32_t imu_hundredths =
    ...
    (void)snprintf((char *)esp32_tx_frame, sizeof(esp32_tx_frame),
        ...
        gps_rtc_sync_valid, (long)gps_rtc_last_drift_seconds,
...
static void RS232_Task(void)
{
  ...
  int n = snprintf(&block[pos], sizeof(block) - pos,
      ...
      "SATS_VISIBLE=%u,SATS_VISIBLE_VALID=%u,SATS_VISIBLE_AGE_MS=%lu,"
      "SATS_USED=%u,SATS_USED_VALID=%u,SATS_USED_AGE_MS=%lu\r\n"
      ...
      latest_gps_satellites_visible, latest_gps_satellites_visible_valid,
      latest_gps_satellites_visible_valid ?
          (now - latest_gps_satellites_visible_ms) : 0U,
      latest_gps_satellites_used, latest_gps_satellites_used_valid,
      latest_gps_satellites_used_valid ?
          (now - latest_gps_satellites_used_ms) : 0U,
      ...
      rtc_text, rtc_time_source, gps_rtc_sync_valid,
      (long)gps_rtc_last_drift_seconds, now, uptime_text);
